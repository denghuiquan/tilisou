    ...
    <%--导航栏结束	--%>
		...
		<!--导入题目表单栏，抽取形成新页面，通过Ajax请求获取，替换上面部分-->
            ...
            <div class="col-md-12 bg-white">
                <div class="box">
                    <h2 class="title_box">更新题目</h2>                    
                    <div class="form-group form-inline">                        
                        <html:form action="/control/timu/manage.do" enctype="multipart/form-data" method="post" >                            
                            <input type="hidden" name="method" value="edit">                            
                            
                            <c:if test="${timu!=null }">
                            <input type="hidden" name="tId" value="${timu.tId }">
					    	<div class="list-group">
									   <a class="list-group-item active">
									      <p><span> 题目ID：<c:out value="${timu.tId }"></c:out></span> 
									      <span class="pull-right"> 发布更新时间：<c:out value="${timu.pubDate }"></c:out></span></p>
									      <p><span> 题目来源：<c:out value="${timu.proSourceDesc }"></c:out></span></p>
									      <p><span> 科目：<span class="label-danger"><c:out value="${timu.subject.name }"></c:out></span></span> 
									      <span> 年级：<span class="label-default"><c:out value="${timu.grade.name }"></c:out></span></span>
									      <span> 题型：<span class="label-success"><c:out value="${timu.type }"></c:out></span></span></p>
									   </a>
									   <a class="list-group-item">
									      ...
									      <p class="list-group-item-text">
									        <img class="img-responsive img-thumbnail" src="${timu.qPicUrl }" alt="${timu.problemContentText }"/>
									      </p><hr/>
									      ...
									      <p class="list-group-item-text">
									        <c:out value="${timu.problemContentText }"></c:out>
									      </p>
									   </a>
									   <a class="list-group-item">
									      ...
									      <p class="list-group-item-text">
									         <img class="img-responsive img-thumbnail" src="${timu.aPicUrl }" alt="解答图片"/>
									      </p>
									   </a>
									</div>
						    	<hr/>
                            ...
                            <div class="bg-primary text-center"><h3 class="">更新内容</h3></div>
                            ...
                            <div class="input-group col-md-10 col-md-offset-1">
                            	...
                            	<input type="text" maxlength="300" class="form-control" id="proSourceDesc" name="proSourceDesc" value="${timu.proSourceDesc }" placeholder="请输入，如：广东2014高考文科数学题" />
                            	<hr/>
                            </div>
                            
                            <div class="input-group col-md-10 col-md-offset-1">
                            	<label for="problemContentText">题目的文字：<span class="text-danger">*注意：如果下面选择了题目图片，这里的题目文字将不会更新到数据库*</span></label>
                            	<textarea rows="6" class="form-control" id="problemContentText" name="problemContentText" maxlength="1000" placeholder="请输入图片中的文字">${timu.problemContentText }</textarea>
                            	<hr/>
                            </div>
                            <br/>
                            ...
                            <div class="text-center col-md-12">
	                            <button type="button" onclick="javascript:editnow(this.form);" class="btn btn-default">确定修改</button>
...
$(document).ready(function(){
	...
	// selected="selected"	
	readySubject('${timu.subject}');
	readyGrade('${timu.grade}');
	readyType('${timu.type}');
		
});

function readySubject(subject){
	var subopts=$("#subject").children().toArray();
	for ( var int = 0; int < subopts.length; int++) {
		var ele = subopts[int];
		if(ele.value==subject){
			ele.selected="selected";
		};		
	}
}
function readyGrade(grade){
	var opts=$("#grade").children().toArray();
...
function readyType(type){
	var opts=$("#type").children().toArray();
	for ( var int = 0; int < opts.length; int++) {
		var ele = opts[int];
		if(ele.value==type){
			ele.selected="selected";
...
function validateEditForm(form){
	...
	if (form.problemContentText.value==null||form.problemContentText.value.trim()=="") {
		if (qPicFile=="") {
			$("#message").html("请填写题目图片识别的文字内容或选择题目图片");
			form.problemContentText.focus();
			return false;
		}
	}
	if (qPicFile && qPicFile!=="") {
