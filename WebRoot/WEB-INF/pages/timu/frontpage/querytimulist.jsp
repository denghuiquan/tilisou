...
<body class="">
    ...
    <div class="row">
        ...
        <div id="timu_display_area" class="col-lg-9 col-md-9 col-sm-9 col-xs-12">
            ...
            <p>共查询到相关题目：<span class="label label-success">${pageView.totalrecord}</span>条  每页显示：<span class="label label-success">${pageView.maxresult}</span>条 总页数：共<span class="label label-success">${pageView.totalpage}</span>页  <span class="pull-right">当前页：第<span class="badge">${pageView.currentpage}</span>页</span> </p>
                        ...
                        <div class="thumbnail">
	                        <div class="text-center">
	                            <a data-toggle="collapse" data-parent="#accordion"
	                                     href="#sPic" class="btn btn-primary">隐藏</a>
	                            <div id="sPic" class="panel-collapse collapse in">                            	
	                                <div class="panel-body">
	                                    <img id="searchimg" class="img-responsive img-thumbnail" src="<%=basePath %>/tmp/tmp.jpg" alt="${ocrword}">
	                                </div>
	                            </div>
	                        </div>
                        </div>
            ...
            <div class="text-center">
                ...
                <div class="input-group pull-right"><form action="<html:rewrite action="/timu/query"/>" method="post" name="timuquery">
					<c:if test="${!empty ocrword }"><input type="hidden" name="word" value="${ocrword}"/></c:if>
					<c:if test="${!empty param.word }"><input type="hidden" name="word" value="${param.word }"/></c:if>
...
function topage(pagenum){
	var form = document.forms["timuquery"];
	form.page.value= pagenum;
	form.submit();
}

//页面跳转方法 ，如果增加查询的排序和过滤功能使用
/* function topage(pagenum){
	window.location.href='<html:rewrite action="/timu/query"/>?sort=${param.sort}&page='+ pagenum+'&grade=${param.grade }&subject=${param.subject }&type=${param.type }';
};  */
